 ...
 * 万能适配器（采用泛型） 主要是针对RecyclerView的适配器<br/></p>
 * <p>
 * 1.抽取出来的模板适配器，任何适配器继承该适配器，就很容易实现适配器功能。<br/>
 * 2.支持多个item<br/>
...
public abstract class BaseRecyclerViewAdapter<T>extends RecyclerView.Adapter<BaseRecyclerViewHolder> {
    ...
    //若果想用自定义的ViewHolder，可以进行自定义，但是必须继承自CommonRecyclerViewHolder
    protected CommonRecyclerViewHolder createViewHolder(View view,int layoutId) {
        return new CommonRecyclerViewHolder(view, layoutId);
    }
     ...
     * @param position
     */
    protected final void onBindItemClickListener(final BaseRecyclerViewHolder holder, final int position) {
