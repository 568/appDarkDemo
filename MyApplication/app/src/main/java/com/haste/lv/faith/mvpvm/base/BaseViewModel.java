 ...
 * 关于有耗时的数据请求时候
 * 1.通过监听Activity、Fragment的生命周期，来自动断开subscription以防止内存泄漏。
 * 2.使用compose操作符绑定容器生命周期。使用bindToLifecycle() 或者 使用bindUntilEvent()
 * 3.注意compose方法需要在subscribeOn方法之后使用
 * 使用说明
 * 1.在网络调用的时候使用.compose(RxUtils.bindToLifecycle(getLifecycleProvider())
 * 2.线程调度使用 .compose(RxUtils.schedulersTransformer())
 */
public class BaseViewModel<T extends AbsRepository> extends AbsViewModel<T> implements IModel{
