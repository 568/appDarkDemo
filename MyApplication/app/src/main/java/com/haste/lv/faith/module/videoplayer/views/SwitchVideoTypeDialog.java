 ...
 * 选取播放的清晰度的选择弹出层，列表方式显示，简单的字符串适配器。
 * 标准
 * -------------
 * 清晰
 * -------------
...
public class SwitchVideoTypeDialog extends Dialog {
    ...
    public void initList(List<SwitchVideoModel> data, OnListItemClickListener onItemClickListener) {
        ...
        View view = inflater.inflate(R.layout.switch_video_dialog_list_layout, null);
        listView = (ListView) view.findViewById(R.id.switch_dialog_list);
        setContentView(view);
        adapter = new ArrayAdapter<>(getContext(), R.layout.switch_video_dialog_list_layout_item, data);
