 ...
 * 因为当分组列表发生变化时，需要及时更新分组列表的组结构{@link SectionRecyclerViewAdapter#mStructures}
 * <p>
 * 1.增加了对XRecyclerView兼容
 * 2.增加了对子项分组支持，主要是针对GridLayoutManager,动态设置getSpanSize，需要覆写getChildSpanSize
 */

public abstract class SectionRecyclerViewAdapter<T>
...
*/
    ...
    //保存分组列表的组结构
    protected ArrayList<SectionStructure> mStructures = new ArrayList<>();
    //数据是否发生变化。如果数据发生变化，要及时更新组结构。
    private boolean isDataChanged;
    private int mTempPosition;
    protected PullToRefreshRecyclerView xRecyclerView;

    @Override
    public void onAttachedToRecyclerView(RecyclerView recyclerView) {
        super.onAttachedToRecyclerView(recyclerView);
        RecyclerView.LayoutManager manager = recyclerView.getLayoutManager();
        xRecyclerView = recyclerView instanceof PullToRefreshRecyclerView ? (PullToRefreshRecyclerView) recyclerView : null;
    ...
    public SectionRecyclerViewAdapter(Context context, List<T> list) {
        mGroups = list;
        if (mGroups == null) {
        ...
        if (type == TYPE_HEADER) {
            if (mOnHeaderClickListener != null) {
                holder.itemView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (mOnHeaderClickListener != null) {
                            mOnHeaderClickListener.onHeaderClick(SectionRecyclerViewAdapter.this,
            ...
            if (mOnFooterClickListener != null) {
                holder.itemView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (mOnFooterClickListener != null) {
                            mOnFooterClickListener.onFooterClick(SectionRecyclerViewAdapter.this,
                                    holder, groupPosition, group);
                        }
                    }
                });
            ...
            if (mOnChildClickListener != null) {
                holder.itemView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        if (mOnChildClickListener != null) {
                            mOnChildClickListener.onChildClick(SectionRecyclerViewAdapter.this,
    ...
    public int judgeType(int position) {
        ...
        for (int i = 0; i < groupCount; i++) {
            SectionStructure structure = mStructures.get(i);
    ...
    private void structureChanged() {
        ...
        for (int i = 0; i < groupCount; i++) {
            mStructures.add(new SectionStructure(hasHeader(i), hasFooter(i), getChildrenCount(i)));
        }
        isDataChanged = false;
    }
    ...
    public int getChildPositionForPosition(int groupPosition, int position) {
        if (groupPosition < mStructures.size()) {
            ...
            SectionStructure structure = mStructures.get(groupPosition);
    ...
    public void insertGroup(int groupPosition) {
        SectionStructure structure = new SectionStructure(hasHeader(groupPosition),
    ...
    public void insertRangeGroup(int groupPosition, int count) {
        ArrayList<SectionStructure> list = new ArrayList<>();
        for (int i = groupPosition; i < count; i++) {
            SectionStructure structure = new SectionStructure(hasHeader(i),
    ...
    public interface OnHeaderClickListener<T> {
        void onHeaderClick(SectionRecyclerViewAdapter adapter, HelperRecyclerViewHolder holder, int groupPosition, T item);
    }

    public interface OnFooterClickListener<T> {
        void onFooterClick(SectionRecyclerViewAdapter adapter, HelperRecyclerViewHolder holder, int groupPosition, T item);
    }

    public interface OnChildClickListener<T> {
        void onChildClick(SectionRecyclerViewAdapter adapter, HelperRecyclerViewHolder holder,
                          int groupPosition, int childPosition, T item);
    }
}
